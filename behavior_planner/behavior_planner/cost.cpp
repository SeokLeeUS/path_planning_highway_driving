...
float goal_distance_cost(const Vehicle& vehicle, const vector<Vehicle>& trajectory, const map<int, vector<Vehicle>>& predictions, map<string, float>& data)
{
	...
	// the farther distance to goal, the better cost.
	// difference between goal land and intended lane/final lane is shorter, the better. 
	else
	{
		cost = 1;
	}
...
float inefficiency_cost(const Vehicle& vehicle, const vector<Vehicle>& trajector, const map<int, vector<Vehicle>>& predictions, map<string, float>& data)
{
