...
// 'this' is to reduce down the number of variables. 
...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>>& predictions) {
	...
	possible_successor_states = successor_states(); // next state estimation 
	vector<vector<Vehicle>> trajectory_for_state;
	vector<Vehicle> instant_traj;
	float cost;
	Vehicle vehicle;
	map<string, float> traj_data;

	for (int i = 0; i < possible_successor_states.size(); ++i)
	{
		instant_traj = generate_trajectory(possible_successor_states[i], predictions);
		trajectory_for_state.push_back(instant_traj);
		cost = calculate_cost(vehicle, predictions, instant_traj);
		traj_data = get_helper_data(vehicle, instant_traj, predictions);
		float goal_distance_weight = goal_distance_cost(vehicle, instant_traj, predictions, traj_data);
		float inefficiency_weight = inefficiency_cost(vehicle, instant_traj, predictions, traj_data);
		float lane_speed_weight = lane_speed(predictions, this->lane);
		float weighted_cost = cost * goal_distance_weight * inefficiency_weight * lane_speed_weight;
		costs.push_back(weighted_cost);
	}

	// min search in vector 
	...
	return trajectory_for_state[minElementIndex];
