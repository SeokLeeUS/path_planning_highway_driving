...
int main() {
	...
	h.onMessage([&map_waypoints_x, &map_waypoints_y, &map_waypoints_s,
		&map_waypoints_dx, &map_waypoints_dy, &lane, &ref_vel] // added pointer address 
				...
				if (length && length > 2 && data[0] == '4' && data[1] == '2') {
					...
					if (s != "") {
						...
						if (event == "telemetry") {
							...
							// next_x_vals/next_y_vals: interpolated points 
							...
							{
								double N = (target_dist / (.02 * ref_vel / 2.24));//ref_vel in mph, to convert it to meter per second
